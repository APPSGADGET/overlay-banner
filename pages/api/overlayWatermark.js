...
export default async function handler(req, res) {
  ...
  */
  ...
  try {
    ...
    let watermark = rawParams.watermark || '';
    let wmSize = parseInt(rawParams.wmSize || '30');
    let wmColor = rawParams.wmColor || '#FFFFFF';
    let wmOpacity = parseFloat(rawParams.wmOpacity || '0.5');
    let noOverlay = rawParams.noOverlay === 'true';
    let unique = rawParams.unique === 'true';
    ...
    // Apply uniqueness randomization if requested
    if (unique) {
      console.log('🎲 Applying image uniqueness randomization...');

      // 1. Geometric randomization: Tiny rotation (-0.05 to +0.05 degrees)
      const randomRotation = (Math.random() * 0.1) - 0.05;

      // 2. Color/Pixel randomization: Subtle gamma shift (1.0 to 1.04)
      // Sharp requires gamma to be between 1.0 and 3.0
      const randomGamma = 1.0 + (Math.random() * 0.04);

      // 3. Subtle brightness/saturation shift (+/- 1%)
      const randomBrightness = 0.99 + (Math.random() * 0.02);
      const randomSaturation = 0.99 + (Math.random() * 0.02);

      processedImage = processedImage
        .rotate(randomRotation, { background: { r: 0, g: 0, b: 0, alpha: 0 } })
        .gamma(randomGamma)
        .modulate({
      ...
      console.log(`🎲 Randomization applied: rot=${randomRotation.toFixed(4)}°, gamma=${randomGamma.toFixed(4)}, bri=${randomBrightness.toFixed(4)}`);
    }
    ...
    const svg = `<?xml version="1.0" encoding="UTF-8"?>
      <svg width="${targetWidth}" height="${svgHeight}" xmlns="http://www.w3.org/2000/svg">
        ...
        ${(design === 'antonTransparent' || design === 'blank' || noOverlay) ? '' :
    ...
    if (design === 'blank' || design === 'antonTransparent') {
      ...
      // Add SVG overlay (title, website, etc.) if not suppressed
      if (!noOverlay) {
        compositeArray.push({
          input: svgBuffer,
          left: 0,
      ...
      if (watermark) {
        ...
        const wmSvg = `
          <svg width="${targetWidth}" height="${wmSize * 2}" xmlns="http://www.w3.org/2000/svg">
            <defs>
            ...
            <text 
              ...
              y="50%" 
              class="wm-text"
            >${escapedWatermark}</text>
          </svg>`;

        compositeArray.push({
          input: Buffer.from(wmSvg),
          top: 20, // Top middle as requested
          left: 0,
          blend: 'over'
        });
        console.log(`🖋️ Watermark added: "${watermark}" at top middle`);
      }

      // Randomize JPEG quality if uniqueness is requested (88-95)
      const finalQuality = unique ? Math.floor(88 + (Math.random() * 8)) : 90;

      let imageFinalizer = processedImage.composite(compositeArray);

      // Add unique metadata for signature if requested
      if (unique) {
        const uniqueId = `OB-${Date.now()}-${Math.random().toString(36).substring(2, 10)}`;
        imageFinalizer = imageFinalizer.withMetadata({
          exif: {
            IFD0: {
              ImageDescription: `Unique Signature: ${uniqueId}`,
              Software: 'Overlay Banner Pro',
              DateTime: new Date().toISOString()
            }
          }
        });
        console.log(`📑 Unique metadata signature added: ${uniqueId}`);
      }

      finalImage = await imageFinalizer
        .jpeg({
          quality: finalQuality,
          chromaSubsampling: unique ? (Math.random() > 0.5 ? '4:4:4' : '4:2:0') : '4:4:4'
        })
        .toBuffer();

      console.log(`✅ Final image generated: ${finalImage.length} bytes (Quality: ${finalQuality})`);
    }
